...
/**
 * 获取参数
 * @param {{ params: Object, query: Object, body: Object }} req
 * @param {Boolean} all
 * @returns {{ params: Object, query: Object, body: Object, all: Object }}
 */
 ...
 * @param {Request} params.req 请求体
 * @param {Boolean} params.all 是否返回全部数据
 * @param {Object}  params.validate { pname: {alias, type, required, range: {in, min, max, reg, schema }, defValue, trim, allowEmptyStr, desc[, detail] } }
 * @param {String}  params.validate.pname 参数名，同时也是验证后使用的变量名
 * @param {String}  params.validate.pname_alias 参数别名，定义该值，前端就用该值传参数来而不是pname
 * @param {String}  params.validate.pname_type 参数类型
 * @param {Boolean} params.validate.pname_required 参数是否必选
 * @param {Object}  params.validate.pname_range 参数范围限制
 * @param {Object}  params.validate.pname_range.in     在指定值列表中
 * @param {Object}  params.validate.pname_range.min    最小值|最短|最早（不同 type 参数 含义有所差异）
 * @param {Object}  params.validate.pname_range.max    最大值|最长|最晚（不同 type 参数 含义有所差异）
 * @param {Object}  params.validate.pname_range.reg    符合指定正则表达式
 * @param {Object}  params.validate.pname_range.schema jsonSchema，针对JSON类型参数有效，使用ajv对参数进行格式控制
 * @param {*}       params.validate.pname_defValue 默认值，没传参数或参数验证出错时生效，此时会将该值赋值到相应参数上
 * @param {Boolean} params.validate.pname_trim          是否去掉参数前后空格字符，默认false
 * @param {Boolean} params.validate.pname_allowEmptyStr 是否允许空串变量 默认不允许， 即 XXXX?YYY= 这种路由 YYY这个参数是否接受，默认这种情况认为没有传该参数
 * @param {String}  params.validate.pname_desc 参数描述 用于出错返回的提示
