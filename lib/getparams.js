 ...
 * @param {boolean} all
 * @returns {{ params: Object, query: Object, body: Object, headers: Object, cookies: Object, all: Object }}
 */
function getParams(req, all) {
  const val = { query: {}, body: {}, params: {}, headers: {}, cookies: {}, all: {} };
  if (all) {
    // 获取全部参数
 ...
 *                defValue, trim:boolean, allowEmptyStr:boolean, allowNull:boolean, desc:string}>} params.validate 参数配置  {@link ParamsConfig}
...
function getValParams(params) {
  ...
  // 处理 retParams 里面的 query、body、params、headers、cookies
...
function getParamKey(allParams, key, from, method) {
  ...
  // 即 headers、cookies 是必须指定了 from 才会获取
  if (from && ['params', 'body', 'query', 'headers', 'cookies'].indexOf(from) >= 0) {
    return `${from}.${key}`;
  }
