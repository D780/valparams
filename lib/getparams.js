...
function getValParams(params) {
  ...
  if (options.equals && options.equals.length) {
    ...
    _.map(options.equals, fields => {
      if (fields.length >= 2) {
        let flag  = true;
        const value = retParams.all[fields[0]];
    ...
    _.map(options.cases, caseInfo => {
      ...
      if (caseInfo.when) {
        for (let i = 0; i < caseInfo.when.length; i++) {
          ...
          if (_.isUndefined(retParams.all[whenField.field])) {
            hit = false;
            break;
          } else if (whenField.value && retParams.all[whenField.field] !== whenField.value) {
            hit = false;
            break;
          }
        }
      ...
      if (!flag) {
        ...
        emOpt.when = _.map(caseInfo.when, whenField => {
          ...
          return validate[whenField.field] ? validate[whenField.field].desc : whenField.field;
        });
